#!/usr/bin/env python3
"""Evaluate ASR via Speaches (OpenAI-compatible API) and update leaderboard CSV."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--speaches-url", default=os.getenv("SPEACHES_BASE_URL", DEFAULT_SPEACHES_URL))
...
def main() -> int:
    ...
    with httpx.Client(base_url=args.speaches_url) as client:
        for dataset in datasets:
            pred_path = None
            if args.save_preds:
                pred_path = Path("results/predictions") / f"{dataset.dataset_id}.jsonl"
